...
public class RetrofitConfig {
	...
	@Bean
	OkHttpClient secureKeyClient(@Value("${service.security.secure-key-username}") String secureKeyUsername,
	...
	@Bean
	Retrofit.Builder secureKeyBuilder(OkHttpClient secureKeyClient, Gson gson) {
	...
	private OkHttpClient.Builder createDefaultClientBuilder() {
		return new OkHttpClient.Builder().connectTimeout(TIMEOUT_IN_SECS, TimeUnit.SECONDS)
				.readTimeout(TIMEOUT_IN_SECS, TimeUnit.SECONDS).writeTimeout(TIMEOUT_IN_SECS, TimeUnit.SECONDS);
	}
	
	@Bean
    IProductServiceRequest productServiceRequest(Retrofit.Builder secureKeyBuilder,
    ...
    @Bean
    ITransactionServiceRequest transactionServiceRequest(Retrofit.Builder secureKeyBuilder,
